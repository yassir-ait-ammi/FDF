...
/*   Updated: 2025/02/07 10:45:14 by yaait-am         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	calculate_scale(t_data *data, t_draw_info *info)
{
	...
	scale_x = (WINDOW_X / data->line_size) * data->zoom;
	scale_y = (WINDOW_Y / data->lines) * data->zoom;
	if (scale_x < scale_y)
		info->scale = scale_x;
	else
		info->scale = scale_y;
...
static void	calculate_offsets(t_data *data, t_draw_info *info)
{
	...
	info->x_offset = (WINDOW_X / 2) - x1 + data->xu;
	info->y_offset = (WINDOW_Y / 2) - y1 + data->yu;
}
