...
/*   Updated: 2025/02/05 12:41:24 by yaait-am         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_init_line(t_d *d, int x1, int x2, int y1, int y2)
{
	d->dx = abs(x2 - x1);
	d->dy = abs(y2 - y1);
	d->sx = help_init(x1, x2, 1);
	d->sy = help_init(y1, y2, 1);
	d->err = d->dx - d->dy;
...
void	draw_line_image(t_image *image, int x1, int y1, int x2, int y2,
			int color1, int color2)
{
	t_d	d;

	ft_init_line(&d, x1, x2, y1, y2);
	extract_rgb(color1, &d.r1, &d.g1, &d.b1);
	extract_rgb(color2, &d.r2, &d.g2, &d.b2);
	while (d.i <= d.steps)
	{
		ft_calc_color(&d);
